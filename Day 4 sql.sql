...
select replace(replace(pass, char(13), ' '), char(10), ' ') as pass
into #test2
from #test
where 
PATINDEX('%iyr:%', pass) > 0 and
PATINDEX('%byr:%', pass) > 0 and
PATINDEX('%hgt:%', pass) > 0 and
PATINDEX('%hcl:%', pass) > 0 and
PATINDEX('%ecl:%', pass) > 0 and
PATINDEX('%eyr:%', pass) > 0 and
PATINDEX('%pid:%', pass) > 0 


drop table if exists #Filtered
select 
	case when PATINDEX('%byr:[0-9][0-9][0-9][0-9] %', pass) > 0  
	then SUBSTRING(pass, PATINDEX('%byr:[0-9][0-9][0-9][0-9] %', pass)+4, 4) else '' end as byr,

	case when PATINDEX('%iyr:[0-9][0-9][0-9][0-9] %', pass) > 0  
	then SUBSTRING(pass, PATINDEX('%iyr:[0-9][0-9][0-9][0-9] %', pass)+4, 4) else '' end as iyr, 

	case when PATINDEX('%eyr:[0-9][0-9][0-9][0-9] %', pass) > 0  
	then SUBSTRING(pass, PATINDEX('%eyr:[0-9][0-9][0-9][0-9] %', pass)+4, 4) else '' end as eyr,

	case when PATINDEX('%hgt:[0-9][0-9][0-9]cm %', pass) > 0		
	then SUBSTRING(pass, PATINDEX('%hgt:[0-9][0-9][0-9]cm %', pass)+4, 3) else '' end as hgtcm,

	case when PATINDEX('%hgt:[0-9][0-9]in %', pass) > 0			
	then SUBSTRING(pass, PATINDEX('%hgt:[0-9][0-9]in %', pass)+4, 2) else '' end as hgtin,

	case when PATINDEX('%hcl:#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f] %', pass) > 0						
	then SUBSTRING(pass, PATINDEX('%hcl:#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f] %', pass)+4, 7) else '' end as hcl,

	case when PATINDEX('%ecl:[a-z][a-z][a-z] %', pass) > 0						
	then SUBSTRING(pass, PATINDEX('%ecl:[a-z][a-z][a-z] %', pass)+4, 3) else '' end as ecl,

	case when PATINDEX('%pid:[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9] %', pass) > 0 
	then 1 else 0 end as pid,
	pass
into #Filtered
from #test2


select count(*) as part1 from #test2
select count(*) as part2 from #Filtered
where	
try_cast(byr as int) between 1920 and 2002 and
try_cast(iyr as int) between 2010 and 2020 and
try_cast(eyr as int) between 2020 and 2030 and
(try_cast(hgtcm as int) between 150 and 193 or try_cast(hgtin as int) between 59 and 76) and
hcl <> '' and
