create table #test (s varchar(500))
...
select 
	right(left(s, CHARINDEX(':', s)-1), 1) as Letter,
	--left(s, CHARINDEX(':', s)-3),
	cast(left(s, CHARINDEX('-', s)-1) as int) as minNum,
	cast(substring(s, CHARINDEX('-', s)+1, 2) as int ) as maxNum,
	ltrim(rtrim(right(s, len(s)-CHARINDEX(':', s)))) as String,
	Len(ltrim(rtrim(right(s, len(s)-CHARINDEX(':', s))))) as StringLen
into #Data
from #test

select 
	sum(case when (StringLen - len(REPLACE(String, Letter, ''))) between minNum and maxNum then 1 else 0 end),
	sum(case when (SUBSTRING(string, minNum, 1) = Letter and SUBSTRING(string, maxNum, 1) <> Letter) or (SUBSTRING(string, minNum, 1) <> Letter and SUBSTRING(string, maxNum, 1) = Letter) then 1 else 0 end)
from #Data
